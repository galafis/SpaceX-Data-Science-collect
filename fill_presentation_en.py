...
def find_placeholder(slide, placeholder_name):
    ...
    for shape in slide.placeholders:
        if (
            (
                ...
                and shape.placeholder_format.type.name == "TITLE"
            )
            or (
                placeholder_name == "SUBTITLE"
                and shape.placeholder_format.type.name == "SUBTITLE"
            )
            or (
                placeholder_name in ["BODY", "CONTENT"]
                and shape.placeholder_format.type.name in ["BODY", "CONTENT", "OBJECT"]
            )
        ):
            print(
                f"DEBUG: Found placeholder by type match: {shape.placeholder_format.type.name}"
            )
            return shape
    ...
    if placeholder_name == "SUBTITLE" and len(slide.placeholders) > 1:
        try:
            ...
            print(
                f"DEBUG: Attempting fallback to placeholder index 1 for {placeholder_name}"
            )
    ...
    print(
        f"Warning: Placeholder 	'{placeholder_name}	' not found reliably on slide."
    )
    return None
    ...
    try:
        if isinstance(layout_name_or_index, str):
            layout_names_to_try = [
                ...
                layout_name_or_index.replace(" ", "_").upper(),
            ]
            ...
            if not slide_layout:
                print(
                    f"Warning: Layout 	'{layout_name_or_index}	' not found by name. Trying common indices."
                )
                if layout_name_or_index == "Title Slide":
                    slide_layout = prs.slide_layouts[0]
                elif layout_name_or_index == "Title and Content":
                    slide_layout = prs.slide_layouts[1]
                elif layout_name_or_index == "Section Header":
            ...
            print(
                f"Warning: Invalid layout identifier 	'{layout_name_or_index}	'. Using default layout."
            )
            slide_layout = prs.slide_layouts[1]

        if not slide_layout:
            print(
                f"Error: Could not find or default to a suitable layout for 	'{title_text}	'. Skipping slide."
            )
            return None
        ...
        if title_shape:
            ...
            print(
                f"Warning: Title placeholder not found for slide 	'{title_text}	'."
            )
        ...
        if body_content:
            ...
            if body_shape:
                ...
                print(
                    f"Warning: Body/Content placeholder not found for slide 	'{title_text}	'."
                )
        return slide
...
try:
    ...
    if len(prs.slides) > 0:
        ...
        if title_slide_content:
            ...
            subtitle_shape = find_placeholder(
                slide1, "SUBTITLE"
            )  # Try finding SUBTITLE first
            ...
            if subtitle_shape:
                if subtitle:
                    subtitle_text = subtitle.replace("<Name>", author_name).replace(
                        "<Date>", "May 2, 2025"
                    )
            ...
            print(
                "Warning: Title Slide content definition not found in slide_content_en."
            )
    ...
    if len(prs.slides) > 1:
        print(
            f"Clearing {len(prs.slides) - 1} slides after the first one before adding new content."
        )
    ...
    prs.save(
        output_path
    )  # output_path is '/home/ubuntu/spacex_presentation_draft_en.pptx'
